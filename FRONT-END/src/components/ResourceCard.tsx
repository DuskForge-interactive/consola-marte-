...
export const ResourceCard = ({ resource, onRequestResupply }: ResourceCardProps) => {
  ...
  return (
    <Card className={`glass-strong p-6 space-y-4 transition-all duration-300 hover:scale-[1.02] ${
      ...
      <div className="flex items-start justify-between">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground font-mono">ID: {resource.id.toUpperCase()}</p>
        </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData}>
            <Line 
              type="monotone" 
              dataKey="value" 
              stroke={isCritical ? 'hsl(var(--critical))' : isWarning ? 'hsl(var(--warning))' : 'hsl(var(--success))'} 
              strokeWidth={2}
              dot={false}
            />
          </LineChart>
